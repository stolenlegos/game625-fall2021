...
public class NPCSearch : MonoBehaviour
{
    ...
    private bool foodInInventory = true;
    ...
    void Update()
    {
        ...
        {
            case NPCMode.wander:
                //what i do when i am in this state
                DoWander();

                //how is transition to a new state
                if (CanSeeCritter())
            ...
            case NPCMode.collect:
                ...
                else if (foodInInventory)
                    npcMode = NPCMode.eatFood;
                break;
            case NPCMode.attack:
                MoveToCritter();
                if (CanSeePickUp())
